import { Component, OnInit, Input, OnChanges, SimpleChanges, ChangeDetectorRef, AfterViewInit, AfterContentChecked, Output, EventEmitter, ViewChild, TemplateRef } from '@angular/core';
...
import { ClientAppSubmissionComponent } from '../client-app-submission.component';
...
@Component({
  standalone: false,
  selector: 'app-client-app-candidate-details',
  templateUrl: './client-app-candidate-details.component.html',
  styleUrls: ['./client-app-candidate-details.component.css']
})
export class ClientAppCandidateDetailsComponent implements OnInit, OnChanges {
  dialogRef: any;
  candidateDubDetail: any;
  totalpages: number;
  currentPage = 1;
  tempCurrentPage = 1;
  canColumns = [{ field: 'sno', header: 'S.No' },
  ...
  filterCodeList: any[] = [];
  userData: UserData;
  @ViewChild('canTemplate', { static: true }) canTemplate: TemplateRef<any>;
...
// directorshipPANMandatory: any;
 directorshipPANMandatory: boolean = false;
  constructor(private router: Router, private cd: ChangeDetectorRef, public screeningService: ScreeningService, public common: CommonService,
    private fb: UntypedFormBuilder, private messageService: MessageService, private dialog: MatDialog, public clientFile: ClientAppSubmissionComponent
  ) {
  ...
  ngOnInit() {
    ...
    this.docHeader = 'Supporting Documents';
  ...
  dateCalc() {
    return this.fb.group({
      ...
      { validator: this.clientFile.dateCompare('periodOfStay', 'periodOfStayTo') },
    );
  }
  ...
  downloadAllFile(candidateId: number): void {
    this.screeningService.downloadAllClientCaseDocument(candidateId).subscribe(
      (resp: any) => {
        if (resp && resp.documents && resp.documents.length > 0) {

          const successDocs = resp.documents.filter((d: any) => d.success);

          if (!successDocs.length) {
            this.showTopCenter('warn', 'Warning', 'No files available to download.');
            return;
          }

          // Download each file one by one with a small delay
          // to avoid browser blocking multiple simultaneous downloads
          successDocs.forEach((doc: any, index: number) => {
            setTimeout(() => {
              this.downloadFile(doc);
            }, index * 500);  // 500ms gap between each download
          });

          this.showTopCenter('success', 'Success',`${successDocs.length} file(s) downloading.`
          );

        } else {
          this.showTopCenter('warn', 'Warning', 'No documents found for this candidate.');
        }
      },
      (error: any) => {
        this.showTopCenter('error', 'Error', 'Failed to download documents.');
      }
    );
  }
  ...
  downloadFile(data) {
    ...
    if(data.docId)
    {
      this.screeningService.downloadClientCaseDocument(data.docId).subscribe(resp => {
        if (resp.document) {
          this.common.downloadDocument(data.docId, resp.document, data.fileName);
        } else {
          alert('file does not exists');
        }
      });
  ...
  isAlExist() {
    ...
    let filter = '';
    if (this.mainForm.get(this.formgroupName).get('candidateId')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'candidateId!=' + this.mainForm.get(this.formgroupName).get('candidateId')?.value;
    }
    if (this.mainForm.get(this.formgroupName).get('firstName')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'firstName==' + this.mainForm.get(this.formgroupName).get('firstName')?.value;
    }
    if (this.mainForm.get('screening')?.get('clientRefNo')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'clientReferenceNo!=' + this.mainForm.get('screening')?.get('clientRefNo')?.value;
    }
    if (this.mainForm.get(this.formgroupName).get('lastName')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'lastName==' + this.mainForm.get(this.formgroupName).get('lastName')?.value;
    }
    if (this.mainForm.get(this.formgroupName).get('middleName')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'middleName==' + this.mainForm.get(this.formgroupName).get('middleName')?.value;
    }
    if (this.mainForm.get(this.formgroupName).get('dob')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'dateOfBirth==' + new DatePipe('en-Us').transform(this.mainForm.get(this.formgroupName).get('dob')?.value, 'yyyy-MM-dd');
      //new DatePipe('en-Us').transform(this.mainForm.get(this.formgroupName).get('dob')?.value, 'dd/MMM/yyyy');
    }
    if (this.mainForm.get(this.formgroupName).get('fatherName')?.value) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'fatherName==' + this.mainForm.get(this.formgroupName).get('fatherName')?.value;
    }
    ...
    if (this.mainForm.get('screening')?.get('clientRefNo')?.valid && this.mainForm.get(this.formgroupName).get('fatherName')?.value && this.mainForm.get(this.formgroupName).get('dob')?.value && this.mainForm.get(this.formgroupName).get('firstName')?.value) {
      this.screeningService.getCandidateInfo(this.userData).subscribe(resp => {
        if (resp) {
          this.candidateDubDetail = resp;
          if (this.candidateDubDetail.length > 0) {
            this.candidateDubDetail.forEach(ele => {
              if (ele.dateOfBirth != null) {
                ele.dateOfBirth = new DatePipe('en-Us').transform(this.common.getTimezoneOffset(ele.dateOfBirth, false), 'dd/MMM/yyyy');
              }
            })
          }
          if (this.candidateDubDetail.length > 0) {
            this.dialogRef = this.dialog.open(this.canTemplate, {
              width: '1200px',
              disableClose: true
            });
          }
