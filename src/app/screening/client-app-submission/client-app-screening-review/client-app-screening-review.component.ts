...
@Component({
  standalone: false,
  selector: 'app-client-app-screening-review',
  templateUrl: './client-app-screening-review.component.html',
  styleUrls: ['./client-app-screening-review.component.css']
})
export class ClientAppScreeningReviewComponent implements OnInit {
 ...
 commondatabaseReviewJson = {
  ...
  ngOnInit() {
    ...
    this.screeningDetails = this.scrService.screeningDetails
          ...
          element.component.forEach((comp, index) => {
            ...
            Newdata.compref.forEach((m) => {
              if (m["key"] === "address" && m.compType === "employmentHR" &&m.fresherFlag!=true) {
                m["value"] =
                  (comp.compRef.address.addressId == this.scrService.defaultAddressId) ? 'NOT PROVIDED' :
              ...
              else if (m["key"] === "address" && comp['screeningComponentInfo']['notApplicableFlag'] != true && (component.compName === this.common.CRIMINAL_CHECK_PCC1 ||
                ...
                if (m.key === 'place') {
                  m.value = comp["compRef"]["address"]["locationName"] ?
                    comp["compRef"]["address"]["locationName"] : comp["compRef"]["address"][m["key"]];
                }
              ...
              if (
                ...
                if(this.scrService.invitationFlag){
            ...
            if ((
              this.scrService.invitationFlag === true) &&
    ...
    if (
      ...
      this.screeningDetails.candidate.document.length > 0
    ) {
      let docList: any[] = [];
      this.scrService.docList.forEach((ele) => {
        const docdata = this.screeningDetails.candidate.document.filter(
