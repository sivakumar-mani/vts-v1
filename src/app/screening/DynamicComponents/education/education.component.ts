...
export class EducationComponent implements OnInit, OnChanges {
  ...
  changeInstitution() {
    setTimeout(() => {
      ...
      if (instnName && this.applicationId !== 3) {
        ...
        if (insObj && this.instAlertCount === 0) {
          if (insObj.researchResultLookupName === 'Fake' && insObj.address.country === 'India' && this.screeningService.forResearchByPassFlag != true) {
            this.instAlertCount++;
            ...
            this.getStatusDetails('genuine');
          } else if ((insObj.researchStatus != this.common.VERIFIEDNUM) && insObj.address.country === 'India') {
            ...
            if (insObj.researchStatus != this.common.VERIFIEDNUM && this.screeningService.forResearchByPassFlag != true) {
              this.instAlertCount++;
  ...
  setInstnName(empInsId, addressId, empInsAddressId) {
    ...
    // const instn = this.instnList.find(x => x.empInsId === empInsId);
          ...
          if (this.applicationId !== 3) {
            ...
            if (insObj) {
              if (insObj.approveStatusLookupName === 'Reject' && insObj.address.country === 'India') {
                ...
                header = 'Alert';
              } else if (insObj.ResearchStatus != this.common.VERIFIEDNUM && insObj.address.country === 'India' && this.screeningService.forResearchByPassFlag != true) {
  ...
  getPaginationValues(filter: any) {
    return {
      ...
      sorts: '-empInsId',
      applyPaging: true,
      indianClientFlag: this.screeningService.indianClientFlag,
      needTotal: true,
      empInsId: 0,
        ...
        if (result) {
          if (result.Responds != null) {
            ...
            this.screeningService.GetInstitutioInfo(values).subscribe(resp => {
              ...
              const objins = this.instnList.find(f => f.empInsId === result.Responds.educationEmployerId && f.address != null
              ...
              this.setInstnName(objins.empInsId, objins.address.addressId, objins.empInsAddressId);
