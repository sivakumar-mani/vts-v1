...
@Component({
  standalone: false,
  selector: 'app-emphr-empsup',
  templateUrl: './emphr-empsup.component.html',
  styleUrls: ['./emphr-empsup.component.css']
})
export class EmphrEmpsupComponent implements OnInit, OnChanges {
  naFlag = false;
  nFlag = false;
  componentName = '';
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.compBaseDetails) {
      ...
      // this.empList = this.baseInfo !== undefined ? this.baseInfo.employer : this.empList;
      this.empList = this.screeningService.employerList;
      // if (this.screeningService.tempEmpHrList.length > 0) {
      //   this.empList = this.empList.filter(x => x.empInsId !== 0);
      //   this.empList.push(this.screeningService.tempEmpHrList[0]);
      // }
      this.setEmpItems('');
  ...
  checkAddress() {
    // this.empSupAdd.next(this.mainForm.get(this.formgroupName).get('employmentSupervisor')?.get('address')?.value);
    // this.empHRAdd.next(this.mainForm.get(this.formgroupName).get('employmentHR')?.get('address')?.value);
  ...
  calcCurrentCompany() {
    if (this.mainForm.get(this.formgroupName).get('employmentHR')?.get('fromDate')?.value &&
      this.mainForm.get(this.formgroupName).get('employmentHR')?.get('toDate')?.value === 'TILL DATE') {
      const val = this.mainForm.get(this.formgroupName).get('employmentHR')?.get('toDate')?.value === 'TILL DATE' ?
        moment(new Date(), 'dd/MM/yyyy') : '';
      const val1 = moment(this.mainForm.get(this.formgroupName).get('employmentHR')?.get('fromDate')?.value, 'dd/MM/yyyy');
      if ((val1 && val1.isValid() === true) && (val && val.isValid() === true)) {
        if (val1 > val) {
          this.mainForm.get(this.formgroupName).get('fromDate')?.setErrors({ incorrect: true });
          this.mainForm.get(this.formgroupName).get('employmentHR')?.get('fromDate')?.markAsTouched();
        } else {
        ...
        if (employerName) {
          ...
          if (empObj && this.empHrAlertCount === 0) {
            ...
            if (!empObj && employerName.trim() !== '') {
              ...
              // msg = 'Do you want to get approval for ' + employerName + ' ?';
              // header = 'Confirmation';
              // this.openDialog(msg, header, 'employmentHR');
            }
  ...
  empSupkeyUpFunction(event, value) {
    if (event.key === 'Enter' || event.key === 'Tab') {
      ...
      this.supervisornameChangr(null, true);
      // this.mainForm.get(this.formgroupName).get('employmentSupervisor.professionalId')?.setValue(0);
      if (value) {
  ...
  setSupName(id: any) {
    ...
    if (prof) {
      ...
      // this.empSupAdd = new BehaviorSubject(prof.address);
      // this.empSupAdd.next(prof.address);
    }
  }
  ...
  if (result?.Responds) {
    ...
    this.master
      .GetInstitutionAndEmpApprovedList(true, this.userData.deptId || 0)
      .subscribe(resp => {

        this.empList = resp;
        this.screeningService.employerList = resp;

        const objins = this.empList.find((f:any) =>
          f.empInsId === result.Responds.educationEmployerId &&
          f.address?.addressId === result.Responds.addressId
        );

        const empHR = this.mainForm.get(this.formgroupName)?.get('employmentHR');

        if (objins && empHR) {
          empHR.get('employerName')?.setValue(objins?.name);
          this.empHRAdd.next(objins?.address);
          empHR.get('address')?.disable();
        }
      });
  }
});
