import {
  ...
  TemplateRef, OnChanges, SimpleChanges, OnDestroy
} from '@angular/core';
...
@Component({
  standalone: false,
  selector: 'app-raise-insufficiency',
  templateUrl: './raise-insufficiency.component.html',
  styleUrls: ['./raise-insufficiency.component.css']
})
export class RaiseInsufficiencyComponent implements OnInit, OnChanges, OnDestroy {
  ...
  // FIX 1: REMOVED @ViewChild('doc') — #doc is inside <ng-template #docAdd>.
  // ng-template content is never in the component's view tree until dialog.open()
  // renders it in a CDK overlay outside this component. ViewChild can never reach it.
  // Use this.docName UntypedFormControl value instead — it's already bound to that input.

  // FIX 2: static: false for #docAdd — safer since this component itself
  // may be rendered inside a parent dialog overlay
  @ViewChild('docAdd', { static: false }) docAdd!: TemplateRef<any>;

  // FIX 3: REMOVED @ViewChild('autosize') — #autosize is inside *ngFor,
  // static: true cannot find elements inside structural directives.
  // It was unused in the component code anyway.
  ...
  userData: any;
  showEntry: any;
  docName: UntypedFormControl = new UntypedFormControl('', Validators.required);
  showInsuffDoc = true;
  ...
  ngOnChanges(change: SimpleChanges) {
    // FIX 5: null guard — docdata can be null/undefined on first change
    if (change['docdata'] && this.docdata) {
      this.docdata.forEach(e => {
        if (e?.insuffDocId) {
          this.selectedDoctID.push(e.insuffDocId);
        }
      });
    }
    if (change['docList'] && this.docList) {
  ...
  getInsuffDetails() {
    this.screeningService.getInsuffDetails(false).subscribe(res => {
      if (res) {
        ...
        if (this.verificationservice.FeeApprovalFlag === true &&
          this.mainForm.get('screeningInsufficiency')?.value.insufficiencyId === 0) {
          this.verificationservice.getAdditionalFeesByScreeningCompId(
            this.mainForm.get(this.formgroupName).get('screeningCompId')?.value
          ).subscribe(resp => {
            if (resp) {
              this.feesList = resp;
              const clientfeesList = this.feesList.filter(f => f.feeApprovalFlag === true);
              if (clientfeesList.length > 0) {
                ...
                const infor = this.insuffReqType.find(
                  x => x.lookUpName.toLowerCase() === this.common.INFO_REQ.toLowerCase()
                )?.lookUpId;
      ...
      const req = this.insuffReqType.find(
        x => x.lookUpName.toLowerCase() === this.common.DOCU_REQ.toLowerCase()
      )?.lookUpId;
      const docinforeq = this.insuffReqType.find(
        x => x.lookUpName.toLowerCase() === this.common.INFO_DOCU.toLowerCase()
      )?.lookUpId;

      if (this.mainForm.get('screeningInsufficiency')?.value.insufficiencyId === 0) {
        const val = (this.mainForm.get('screeningInsufficiency.insuffDetail') as UntypedFormArray).controls;
        val.forEach(element => { element.get('comments')?.setValue(''); });
        this.mainForm.get('screeningInsufficiency.requiredLookupId')?.setValue(undefined);
      }
      ...
      if (this.mainForm.get('screeningInsufficiency.requiredLookupId')?.value === null ||
        ...
        const currentVal = this.mainForm.get(this.formgroupName + '.requiredLookupId')?.value;
        if (currentVal === req) {
          this.showInsuffDoc = true;
        } else if (currentVal === docinforeq) {
          this.showInsuffDoc = true;
      ...
      if (this.mainForm.get('screeningInsufficiency.levelLookupId')?.value === null ||
        ...
        this.mainForm.get('screeningInsufficiency.levelLookupId')?.setValue(
          this.mainForm.get('screeningInsufficiency.levelLookupId')?.value
        );
      }
  ...
  saveInsuffDoc() {
    // FIX 1: use this.docName.value (UntypedFormControl) instead of this.doc.nativeElement.value
    // this.doc was a @ViewChild for #doc inside <ng-template> — always undefined
    // because ng-template content renders in a CDK overlay outside the component view.
    // docName UntypedFormControl is already two-way bound to that input via [formControl]="docName".
    const value = (this.docName.value ?? '').trim();

    // FIX 4: null guard — value could be null if docName was never set
    if (!value) {
      this.showTopCenter('warn', 'Failure Message',
        'Failed to save. Please Provide a valid Insufficiency document Name');
      this.docName.setValue('');
      return;
    }

    const alreadyExists = this.insuffDocArr.length > 0 &&
      this.insuffDocArr.some(
        x => x.description.toLowerCase().trim() === value.toLowerCase()
      );

    if (alreadyExists) {
      this.showTopCenter('warn', 'Failure Message',
        'Failed to save. Insufficiency document Name is already available');
      this.docName.setValue('');
      return;
    }
    ...
    this.insuffDocList = [{
      insuffDocId: 0,
      insuffDocTransId: 0,
      componentId: this.compBaseDetails?.compId,
      description: value,
    ...
    this.screeningService.addInsuffDocument(this.insuffDocList).subscribe(resp => {
      if (resp) {
        ...
        // FIX 1: reset via UntypedFormControl instead of nativeElement.value = ''
        this.docName.setValue('');
        this.getInsuffDoc();
  ...
  getInsuffDoc() {
    this.screeningService.getInsuffDocument(this.compBaseDetails?.compId).subscribe(resp => {
      ...
      this.docdata = Object.assign(
        [], this.mainForm.get(this.formgroupName + '.insuffDocument')?.value ?? []
      );
      this.docdata.forEach(e => {
        if (e?.insuffDocId) { this.selectedDoctID.push(e.insuffDocId); }
      });
      this.select();
    });
  }
  ...
  showNotification(severity: string, summary: string, message: string) {
    this.shared.emitChange({ severity, summary, detail: message });
  }

  showDocEntry() { this.showEntry = true; }
  ...
  getRequiredType(name: string) {
    ...
    if (remarkDetail.length > 1) {
      ...
      while (ind >= 1) { remarkDetail.removeAt(ind); ind--; }
    }
  ...
  addInsufDesc() {
    ...
    if (data.length > 0) {
      ...
      data.forEach((e: any) => { insRemarksText += '\n' + e.description + ','; });
  ...
  remarkblur(formgrp: UntypedFormGroup, value: any) {
