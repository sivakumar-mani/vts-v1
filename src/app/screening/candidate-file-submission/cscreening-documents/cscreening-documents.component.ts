...
@Component({
  standalone: false,
  selector: 'app-cscreening-documents',
  templateUrl: './cscreening-documents.component.html',
  styleUrls: ['./cscreening-documents.component.css'],
  animations: [
...
export class CscreeningDocumentsComponent implements OnInit {
  size = 0;
  ...
  // digitalPVReport() {
  //   this.verification.digitalAddressCheckPVReport(this.userData).subscribe(res => {
  //     if (res) {
  //       this.digitalAddressPVReportList = res;
  //       if (this.verification.tempData && this.verification.tempData.verificationScreeningDet.screeningCompId > 0) {
  //         this.addressdocument = this.digitalAddressPVReportList.filter(x => x.screeningCompId === this.verification.tempData.verificationScreeningDet.screeningCompId);
  //         if (this.addressdocument.length > 0) {
  //           if (this.addressdocument[0].document.length > 0) {
  //             this.responseConfirmation.setValue(this.verification.tempData.verificationTransBindDet.
  //               receivedConfirmationType.find(x => x.lookUpName === 'Digital Mode').lookUpId);
  //             for (let i = 0; i < this.addressdocument[0].document.length; i++) {
  //               const screenDoc = new ScreeningDocument();
  //               if (this.header === 'response') {
  //                 screenDoc.fileName = this.addressdocument[0].document[i].fileName;
  ...
  //                 screenDoc.responseConfirmationId = this.verification.tempData.verificationTransBindDet.
  //                   receivedConfirmationType.find(x => x.lookUpName === 'Digital Mode').lookUpId;
  //               }
        ...
        if (event.target.files.length > 0) {
          ...
          if (ext === 'png' || ext === 'jpg' || ext === 'JPG' || ext === 'jpeg' || ext === 'gif' || ext === 'psd' || ext === 'bmp') {
            if (event.target.files[0].size <= 10240000) {
              ...
              // const dialogRef = this.dialog.open(this.imgDialog, {
              //   width: '600px',
              //   disableClose: true
              // });
    ...
    reader.onloadend = () => {
      ...
      if (typeof result === 'string') {
        this.imageCompress.compressFile(result, -1, 75, 50).then(
          compressedResult => {
            this.imageChangedEvent = compressedResult;
          }
        );
      }
    };
