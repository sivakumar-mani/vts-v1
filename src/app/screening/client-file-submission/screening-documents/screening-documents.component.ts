...
@Component({
  standalone: false,
  selector: 'app-screening-documents',
  templateUrl: './screening-documents.component.html',
  styleUrls: ['./screening-documents.component.css'],
  animations: [
...
export class ScreeningDocumentsComponent implements OnInit {
  size = 0;
  ...
  ngOnInit() {
    ...
    if (this.mainForm) {
      if (this.header === 'response' || this.header === 'reOpenDoc' || this.header === 'recievedInfo' || this.header === 'Gap Reason Supporting Documents') {
    ...
    if (this.formgroupName != 'document' && this.mainForm) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.validationFlag && this.mainForm) {
      if (!this.validationFlag) {
        this.mainForm.get(this.formgroupName)?.get('componentDocument')?.clearValidators();
      } else {
        if (this.mainForm.get(this.formgroupName)?.get('componentDocument')?.value?.length == 0) {
    ...
    if (this.showDocType) {
      ...
      if (this.responseConfirmation.value > 0) {
        if (event.target.files.length > 0) {
          ...
          if (ext === 'png' || ext === 'jpg' || ext === 'JPG' || ext === 'jpeg' || ext === 'gif' || ext === 'psd' || ext === 'bmp') {
  ...
  openUploadDoc(event: any) {
    ...
    const fileSizeInBytes = this.size;
    ...
    for (let i = 0; i < this.frmArray.length; i++) {
      if (this.frmArray.value[i].document.length != 0) {
        let fSize = this.frmArray.value[i].document.size;
        this.size += fSize;
        if (this.header === 'reOpenDoc' && parseFloat(formattedSize) > 5) {
          this.size = 0;
          this.showTopCenter('warn', 'Failure Message', 'The uploaded file size should be less than 5 MB');
          return;
        }
      }
    }
    if (this.frmArray.length === 0 && this.header === 'reOpenDoc') {
      if (this.header === 'reOpenDoc' && parseFloat(formattedSize) > 5) {
        this.size = 0;
        this.showTopCenter('warn', 'Failure Message', 'The uploaded file size should be less than 5 MB');
        return;
    ...
    if (!fileSize) {
      this.size = 0;
      this.showTopCenter('warn', 'Failure Message', this.header === 'reOpenDoc' ? 'the overall file size should be less than 150 MB' : 'the overall file size should be less than 5 MB');
      return;
    } else {
      let i = 0;
      for (i; i < event.target.files.length; i++) {
        ...
        if (this.header === 'response') {
          ...
          this.documentType.setValue(screenDoc.docTypeId);
        } else if (this.header === 'reOpenDoc') {
          const receivedInfoDocLookupId = this.verificationTransBindDet.reOpenDocLookupId;
          screenDoc.docTypeId = receivedInfoDocLookupId;
          this.documentType.setValue(screenDoc.docTypeId);
        } else {
        ...
        if (this.header === 'response' || this.header === 'Annexure Upload') {
          ...
          if (this.userData.applicationId === 3 && this.screeningService.compData.compName.toUpperCase() === this.common.CV_VALIDATION) {
            if (ext === 'pdf' || ext === 'docx' || ext === 'doc') {
              ...
              this.showTopCenter('warn', 'Failure Message',
                'Please upload a valid file,' + ' Acceptable file Formats : .pdf, .docx, .doc');
            }
          } else {
  ...
  bytesToSize(bytes: any) {
    ...
    if (i === 2 && this.header !== 'reOpenDoc') {
      return total > 5 ? false : true;
    }
    if (i === 2 && this.header === 'reOpenDoc') {
      return total > 150 ? false : true;
    } else if (i > 2) {
      return false;
    } else {
      return true;
