...
@Component({
  standalone: false,
  selector: 'app-screening-components',
  templateUrl: './screening-components.component.html',
  styleUrls: ['./screening-components.component.css']
})
export class ScreeningComponentsComponent implements OnInit {
  fresherFlag = false;
  ...
  ngOnInit() {
    ...
    this.common.dob = '';
    this.common.dob = this.mainForm.get('candidate')?.get('dob')?.value;
  }
  ...
  screeningComponentStatusDetails(compId: any) {
    ...
    if ((this.componentName !== this.common.EMPLOYMENT_HR && this.componentName !== this.common.CURRENT_EMPLOYMENT_HR && this.componentName !== this.common.PREVIOUS_EMPLOYMENT_HR && this.componentName !== this.common.ADDRESS && this.screeningService.caseFlag == true && this.componentName !== this.common.DRUG_TEST && this.componentName !== this.common.EDUCATION && this.componentName !== this.common.CRIMINAL_DATABASE && this.componentName !== this.common.DATABASE_CONDUCT && this.componentName !== this.common.DATABASE_ADVERSE_MEDIA) || this.screeningService.caseFlag != true) {
      ...
      if (data.subCompFlag) {
        ...
        for (let k = 0; k < this.selectedComponent.noOfComponent; k++) {
          ...
          if (this.selectedComponent.noOfComponent === 1 || k === 0) {
            ...
            } else if (this.selectedComponent.compName.toUpperCase() === this.common.EMPLOYMENT_HR || this.selectedComponent.compName.toUpperCase() === this.common.CURRENT_EMPLOYMENT_HR) {
          ...
          if (this.userdata.applicationId !== 3) {
            if (this.screeningService.caseFlag
              ...
              if (data.preQCApproveFlag || data.preQCRejectFlag) {
                ...
                if (this.mainForm.get('invitationFlag')?.value == true && data.submittedFlag != true && data.preQCApproveFlag != true && this.screeningService.caseFlagType === this.common.PREQCCASE) {
                  this.fileBtnFlag = false;
                  this.disablesubmitebtn = false;
                }
                else {
              ...
              if ((data.submittedFlag === true && data.compRef.fresherFlag === true && this.componentName.toLowerCase() === this.common.EMPLOYMENT_HR.toLowerCase()) ||
                data.submittedFlag === true && data.compRef.fresherFlag === true && this.componentName.toLowerCase() === this.common.CURRENT_EMPLOYMENT_HR.toLowerCase()) {
                this.fileBtnFlag = false;
                this.disablesubmitebtn = false;
              }
            }
  ...
  getPreQcOrNot(i: any) {
    ...
    const ret = data.some(s => (s.screeningComponentInfo.deqcFlag === true || s.screeningComponentInfo.insuffRaisedFlag === true) && s.preQCApproveFlag === false);
    return ret ? i : -1;
  }
  getValidorNot(index, compName): boolean {
    ...
    if (compName === this.common.GAPREASON) {
      ...
      return reasonArray.valid;
    }
    if (compName === this.common.EMPLOYMENT_HR || compName === this.common.CURRENT_EMPLOYMENT_HR) {
    ...
    if (singleComp.controls.some(x => x.valid === true) && this.mainForm.get('invitationFlag')?.value === true) {
      if (compName === this.common.ADDRESS && this.userdata.applicationId === 3) {
  ...
  submitComp(type, index) {
    ...
    if (frmGroup.valid) {
      ...
      if (this.userdata.applicationId === 3 && this.notApplicableFlag != true && type && type !== 'Gap Reason' &&
        ...
        type !== 'Gap Check' && type !== "SSN TRACE" && type !== "NATIONWIDE SEX OFFENDER" && type !== "NATIONWIDE SEX OFFENDER 5 YEARS"
        && type !== "CRIMINAL SEARCH STATEWIDE 10 YEARS" && type !== 'CRIMINAL - FEDERAL NATIONWIDE 10 YEARS'
        && type !== 'FEDERAL DISTRICT SEARCH 10 YEARS' && type !== 'CRIMINAL - FELONY & MISDEMEANOR 10 YEARS'
  ...
  getRejectedComments(type, index) {
    ...
    if (this.screeningService.caseFlagType === this.common.REOPEN) {
      this.screeningService
        .GetReopenCommentsByScreenCompId(this.screeningService.screeningCompId)
        .subscribe((res: any) => {
          if (res) {
            this.rejectedCommentsList = res;
          }
