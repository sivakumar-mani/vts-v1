...
import { ScreeningComponentsComponent } from "./screening-components/screening-components.component";
...
@Component({
  standalone: false,
  selector: "app-client-file-submission",
  templateUrl: "./client-file-submission.component.html",
  styleUrls: ["./client-file-submission.component.css"],
})
export class ClientFileSubmissionComponent
  ...
  @ViewChild(ScreeningComponentsComponent)
  viewScreeningComp: ScreeningComponentsComponent;
  ...
  getstatus(value: any) {
    if (this.screeningService.StatusDeFlag === true) {
      ...
      const eduflag = value.screeningCaseComponent.filter(
      ...
      const drugFlag = value.screeningCaseComponent.filter(
      ...
      const databaseFlag = value.screeningCaseComponent.filter(
        (f) =>
          (f.compName.toLowerCase() ===
          this.common.CRIMINAL_DATABASE.toLowerCase()) || (f.compName.toLowerCase() ===
          this.common.DATABASE_CONDUCT.toLowerCase()) || (f.compName.toLowerCase() ===
          this.common.DATABASE_ADVERSE_MEDIA.toLowerCase())
      );
    ...
    this.screeningService
      ...
      .subscribe((resp) => {
        if (resp) {
          ...
          this.screeningService.clientId = resp.screening.clientId;
          this.screeningService.clientName = resp.screening.clientName;
          this.screeningService.ClientCategoryId = resp.screening.clientCategoryId;
          this.screeningService.forResearchByPassFlag = resp.screening.forResearchByPassFlag
          this.screeningService.enableMutiJcrFlag = resp.screeningComponent[0].component[0].enbMultiJCRFlag;
          this.screeningService.compId = resp.screeningCaseComponent[0].compId;
          this.screeningService.enableNAFlag = resp.candidate.enableNAFlag;
          this.screeningService.IqcByPassFlag = resp.candidate.iqcByPassFlag;
          this.screeningService.defaultAddressId = resp.candidate.defaultAddressId;
          this.screeningService.indianClientFlag = resp.candidate.indianClientFlag;
          this.screeningService.invitationFlag = !resp.invitationFlag ? resp.showClientSuspectFlag : '';
          this.screeningService.dAinvitationFlag =resp.screening.invitationFlag;
          if (this.screeningService.caseFlagType === this.common.INSUFFCLEARANCE) {
            resp.screeningComponent[0].component[0].screeningComponentInfo.compInitiationDate = new Date();
          }
          ...
          this.breadcrumbFlags.btnReset = true;
          this.btnApprove = false;
          this.common.dateRestrictionInPOS = resp.screening.dateRestrictionInPOS;
        }
      });
  }
        ...
        if (resp) {
          ...
          if (this.snFlag == true) {
            ...
            if (selectedChecks.subCompFlag && this.applicationId != 3) {
              ...
              noofComp =
                this.screeningService.compData.screeningSubComponent.filter(
                  (f) => f.subCompName !== "Current Address"
                ).length;
            } else {
  ...
  getCandidateCase() {
    this.showGrid = false;
    this.screeningService
      .getUserCaseScreeningDetails(this.userData.userId)
      .subscribe((resp) => {
        if (resp) {
          this.bindCaseDetail(resp);
          if (this.userData.applicationId === 3) {
            this.userData.firstName =
              resp.candidate.firstName +
              " " +
              (resp.candidate.middleName ? resp.candidate.middleName : "") +
              " " +
              (resp.candidate.lastName ? resp.candidate.lastName : "");
            if (this.step1 === 0) {
    ...
    if (this.snFlag == true) {
      ...
      if (selectedChecks.subCompFlag && this.applicationId != 3) {
        ...
        noofComp = this.screeningService.compData.screeningSubComponent.filter(
          (f) => f.subCompName !== "Current Address"
        ).length;
      } else {
        noofComp = this.screeningService.compData.noOfComponent;
  ...
  bindCaseDetail(resp: any) {
    ...
    resp.candidate.dob = resp.candidate.dob != null ? this.datePipe.transform(resp.candidate.dob, "dd/MMM/yyyy") : '';
    ...
    resp.screeningCaseComponent.forEach((el) => {
      if (el.compName.toLowerCase() !== "voter id" ||
        ...
        el.compName.toLowerCase() !== this.common.CURRENT_EMPLOYMENT_HR ||
        el.compName.toUpperCase() !== this.common.PREVIOUS_EMPLOYMENT_HR ||
        el.compName.toUpperCase() !== "database") {
    ...
    for (let i = 0; this.screeningService.componentList.length > i; i++) {
      ...
      if (frmIndex > -1) {
        ...
        if (this.screeningService.componentList[i].subCompFlag) {
            ...
            if (subComp.length > 0) {
              for (let l = 0; subComp.length > l; l++) {
                ...
                if (subCompdata.compRef.screeningCompId !== null) {
                  ...
                  if (this.screeningService.componentList[0].compName.toString().toUpperCase() == 'ADDRESS' && this.screeningService.ClientCategoryId == 4) {
                    if (
                  ...
                  if (subCompdata.compRef.address !== null) {
                    ...
                    if (this.screeningDetails1.screeningComponent[frmIndex].component[l].compRef.address != undefined && subCompdata.screeningComponentInfo.notApplicableFlag != true) {
                      ...
                      if ((this.screeningService.compId == this.common.EMPLOYMENT_HRId ||
                        this.screeningService.compId == this.common.CURRENT_EMPLOYMENT_HRId ||
                        this.screeningService.compId == this.common.PREVIOUS_EMPLOYMENT_HRId || this.screeningService.compId == this.common.EDUCATIONId) && subCompdata.compRef.isVerified == false) {
          ...
          for (let j = 0; this.screeningDetails1.screeningComponent[frmIndex].component
            .length > j; j++) {
            if (this.screeningDetails1.screeningComponent[frmIndex].compId == this.common.BANK_STATEMENTId) {
            ...
            if (this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address != undefined && this.screeningDetails1.screeningComponent[frmIndex].component[j].screeningComponentInfo.notApplicableFlag != true) {
              if (this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address != undefined) {
                ...
                //   const addressPos =
                //     address != null
                //       ? (address.get("addressPos") as UntypedFormArray)
                //       : (address1.get("addressPos") as UntypedFormArray);

                //   if (
                //     this.screeningDetails1.screeningComponent[frmIndex]
                //       .component[j].compRef.address[0]? this.screeningDetails1.screeningComponent[frmIndex]
                //       .component[j].compRef.address[0].addressPos: this.screeningDetails1.screeningComponent[frmIndex]
                //       .component[j].compRef.address.addressPos !== null
                //   ) {
                //     timer(0, 25).subscribe((x) => this.createPosForm(
                //       addressPos,
                //       this.screeningDetails1.screeningComponent[frmIndex]
                //       .component[j].compRef.address[0]!=undefined? this.screeningDetails1.screeningComponent[frmIndex]
                //       .component[j].compRef.address[0].addressPos:this.screeningDetails1.screeningComponent[frmIndex]
              ...
              //   if (address != null) {
              //     if (    this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address.addressPos.length == 0 ||
              //       this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address.addressPos == undefined
              //     ) {
              //       if (address != null) {
              //         timer(0, 25).subscribe((x) => address.addControl(
              //           "addressPos",
              ...
              //         );
              //       } else {
              //         timer(0, 25).subscribe((x) =>  address1.addControl(
              //           "addressPos",
              //           this.common.initPosForm(
              //             this.screeningDetails1.screeningComponent[frmIndex]
              //             .component[j].compRef.address[0]!=undefined? this.screeningDetails1.screeningComponent[frmIndex]
            ...
            //}
            if (this.screeningService.compId == this.common.GAP_VERIFICATION_ID && this.screeningService.ClientCategoryId == 4 && this.screeningDetails1.screeningComponent[frmIndex].component[j].gapDetails.length > 0) {
            ...
            const scrcompDoc = compFrmGroup.get("screeningComponentInfo.componentDocument") as UntypedFormArray;
            if (this.screeningDetails1.screeningComponent[frmIndex].component[j].screeningComponentInfo) {
              const screeningComponentInfo =
                this.screeningDetails1.screeningComponent[frmIndex].component[j].screeningComponentInfo;
              let screeningInsufficiency = this.screeningDetails1.screeningComponent[frmIndex].component[j].screeningInsufficiency;
              this.createDocForm(
                scrcompDoc,
                screeningComponentInfo.componentDocument
              );
              ...
              if (this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address == undefined ||
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address == null) {
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address = "";
              } else { // this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.address = "";
              }
              if ((this.screeningService.compId == this.common.EMPLOYMENT_HRId ||
                this.screeningService.compId == this.common.CURRENT_EMPLOYMENT_HRId ||
                this.screeningService.compId == this.common.PREVIOUS_EMPLOYMENT_HRId || this.screeningService.compId == this.common.EDUCATIONId) && this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.isVerified == false) {
                addressFrmGroup.addControl('isClientSuspectFlag', new UntypedFormControl())
              }
              compFrmGroup.patchValue({
                active: false,
                compRef: this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef,
                submittedFlag: this.screeningDetails1.screeningComponent[frmIndex].component[j].submittedFlag,
                preQCApproveFlag: this.screeningDetails1.screeningComponent[frmIndex].component[j].preQCApproveFlag,
                preQCRejectFlag: this.screeningDetails1.screeningComponent[frmIndex].component[j].preQCRejectFlag,
                rejectComments: this.screeningDetails1.screeningComponent[frmIndex].component[j].rejectComments,
                qcRejectComments: this.screeningDetails1.screeningComponent[frmIndex].component[j].qcRejectComments,
                addedByCandidateFlag: this.screeningDetails1.screeningComponent[frmIndex].component[j].addedByCandidateFlag,
                componentCustomFields: this.screeningDetails1.screeningComponent[frmIndex].component[j].componentCustomFields,
                periodOfStayAddress: this.screeningDetails1.screeningComponent[frmIndex].component[j].periodOfStayAddress
                  ? this.screeningDetails1.screeningComponent[frmIndex].component[j].periodOfStayAddress : [],
              });
              ...
              compFrmGroup["controls"]["compRef"]["controls"]["screeningCompId"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.screeningCompId
              );
              compFrmGroup["controls"]["compRef"]["controls"]["screeningEmployeeId"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.screeningEmployeeId
              );
              compFrmGroup["controls"]["compRef"]["controls"]["fresherFlag"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.fresherFlag
              );
              compFrmGroup["controls"]["active"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].active
              );
              compFrmGroup["controls"]["submittedFlag"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].submittedFlag
              );
              compFrmGroup["controls"]["preQCApproveFlag"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].preQCApproveFlag
              );
              compFrmGroup["controls"]["preQCRejectFlag"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].preQCRejectFlag
              );
              compFrmGroup["controls"]["screeningComponentInfo"]["controls"]["screeningCompId"].setValue(
                this.screeningDetails1.screeningComponent[frmIndex].component[j].compRef.screeningCompId
              );
            }
            if (this.screeningDetails1.screeningComponent[frmIndex].criminalCheckCount > 0) {
              const count =
                this.screeningDetails1.screeningComponent[frmIndex]
                  .criminalCheckCount;
            ...
            if (this.screeningDetails1.screeningComponent[frmIndex].compId === 13) {
  ...
  compIniDate(date: any) {
    ...
    if (this.screeningService.invitationFlag == true) {
      if (date == null) {
        compDate = this.fileSubmission.get('candidate.caseInititationDate')?.value != null ?
          this.fileSubmission.get('candidate.caseInititationDate')?.value : new Date();
      } else {
  ...
  initFormGroup() {
    this.fileSubmission = this.fb.group({
      ...
      screening:
        this.screeningService.caseFlagType === this.common.NEWCASE ||
          this.manualFilesubmissionRE
          ? this.initmanualClientDetailForm()
          : this.initClientDetailForm(),
    ...
    return this.fb.group(
      {
        ...
        address:
          this.userData.applicationId === 3
            ? this.initCommonAddress(
              this.userData.applicationId === 3 ? true : false
            )
  ...
  initComponentForm(
    ...
    if (this.screeningService.componentList.length > 0) {
      ...
      if (compNameList.length > 0) {
        if (
          ...
          compNameList[0].compName !== "Gap Reason" &&
          compNameList[0].compName !== "Database" &&
          compNameList[0].compName !== "Address" &&
          compNameList[0].compName !== "Drug Test" &&
          compNameList[0].compName !== "Emergency Contact Verification" &&
          compNameList[0].compName !== "Employment (Supervisor)" &&
          compNameList[0].compName !== "Gap Verification" &&
          compNameList[0].compName !== "Reference Check" &&
          compNameList[0].compName !== "Gap Check" &&
          subName.length === 0
        ) {
    ...
    return new UntypedFormGroup({
      ...
      screenStatusId: new UntypedFormControl(
        0,
        this.applicationId !== 3 ? [required, min] : required
      ),
  ...
  initAddressForm(): UntypedFormGroup {
    ...
    return this.fb.group({
      ...
      // periodOfStay: new UntypedFormControl('', othervalidation),
      // periodOfStayTo: new UntypedFormControl('', validation),
    ...
    if (this.applicationId !== 3) {
      ...
      if (
        ...
        this.openconfirmationDialog(
          'Thank you for submitting the online application for background verification! You will not be able to edit any detail or upload any document after Clicking "Save & Submit". Click to Continue.',
          this.getPaymentFlag()
        );
      } else {
        ...
        if (this.fileSubmission.get("candidate").valid) {
          ...
          for (
            let i = 0;
            i < this.fileSubmission.value.screeningComponent.length;
            i++
          ) {
            if (
              ...
              const compCount = rawvalues.screeningComponent[
                i
              ...
              count =
                ...
                this.fileSubmission.value.screeningComponent[i].component
                  .length;
              this.validationExp(
                this.fileSubmission.value.screeningComponent[i].compId,
                i
              );
            }
          }
          ...
          if (
            (compLength > 0 && compLength !== count && checkFlag === true) ||
            (compLength === count &&
              this.fileSubmission.get("candidate.consentLookupName").value ===
              "EConsent" &&
              this.fileSubmission.get("candidate.iagreeFlag").value &&
            ...
            //   const educationIndex = this.screeningService.componentList.findIndex(f => f.
            // compName.toUpperCase() === this.common.EDUCATION);
            //   const empHRIndex = this.screeningService.componentList.
            // findIndex(f => f.compName.toUpperCase() === this.common.EMPLOYMENT_HR);
            ...
            if (this.screeningService.gapReason.length > 0) {
              const educationIndex =
                this.screeningService.componentList.findIndex(
              ...
              const screeningcomp = this.fileSubmission.get(
                "screeningComponent"
              ) as UntypedFormArray;
              if (educationIndex > -1 && empHRIndex > -1) {
                if (
                  screeningcomp.controls[educationIndex].valid &&
                  screeningcomp.controls[empHRIndex].valid
                ) {
                  this.fileSubmission.get("gapReason").markAllAsTouched();
                  if (this.fileSubmission.get("gapReason").invalid) {
            ...
            const referenceComp = this.screeningService.componentList.find(
              (s) => s.compName === this.common.REFERENCE_CHECK
            );
            if (referenceComp) {
              const scrcompvalue =
                this.fileSubmission.get("screeningComponent").value;
              const referencecheckcompindex = scrcompvalue.findIndex(
                (fi) => fi.compId === referenceComp.compId
              );
              const refcompvalue = scrcompvalue.find(
                (fi) => fi.compId === referenceComp.compId
              );
              if (refcompvalue) {
                const validdata = refcompvalue.component.filter(
                  (f) => f.active === true
                );
                const screeningComp = this.fileSubmission.get(
                  "screeningComponent"
                ) as UntypedFormArray;
                const validScreeningComp = screeningComp.controls.filter(
                  (fc, fi) => {
                ...
                const rondupIndex =
                  this.screeningService.gapReason.length > 0 ? 2 : 1;
                if (
                  screeningComp.controls.length ===
                  validScreeningComp.length + 1
                ) {
                  if (validdata.length >= 3 && this.popHide === true) {
                    ...
                    this.showNotification(
                      "warn",
                      "Failed to save",
                      "At least three reference check should be enter data."
                    );
                    return false;
                  }
            ...
            if (
              this.fileSubmission.get("screeningComponent").valid &&
              checkFlag === true
            ) {
              this.step1 = this.stepHiddenFlag ? 2 : 1;
              this.stepperChange(this.step1);
  ...
  getGapinEducation() {
    let count = 0;
    const formArrayRawValue =
      this.fileSubmission.get("screeningComponent").value;
    const educationId = this.screeningService.componentList.find(
      (f) => f.compName === this.common.EDUCATION
    );
    const education = formArrayRawValue.find(
      (f1) => f1.compId === educationId.compId
    );
      ...
      while (i > -1) {
        ...
        if (i !== preIndex) {
          const startDate = this.common.convertDate(
            education.component[i].compRef.courseCompletion
          );
          const endDate = this.common.convertDate(
            education.component[preIndex].compRef.courseStart
          );
  ...
  getGapinEduEmp(tyep, start, end) {
    ...
    const formArrayRawValue =
      this.fileSubmission.get("screeningComponent").value;
    const compId = this.screeningService.componentList.find(
      (f) => f.compName === tyep
    );
    const compData = formArrayRawValue.find(
      (f1) => f1.compId === compId.compId
    );
    ...
    if (compData) {
      while (i > -1) {
        ...
        if (i !== preIndex) {
          ...
          let endDate: any;
          if (
            compData.component[i] &&
            compData.component[i].compRef &&
            compData.component[preIndex].compRef
          ) {
            startDate = this.common.convertDate(
              compData.component[i].compRef[end]
            );
            endDate = this.common.convertDate(
              compData.component[preIndex].compRef[start]
            );
          }
          if (startDate !== "Invalid Date" && endDate !== "Invalid Date") {
            const absstartdate: any = new Date(
    ...
    if (this.userData.applicationId === 3) {
      this.screeningService.componentList.forEach((element, index) => {
        if (element.subCompFlag) {
          ...
          if (subcomp) {
            const compIndex =
              this.screeningDetails.screeningComponent.findIndex(
                (i) => i.compId === subcomp.compId
              );
            const subCombIndex = this.screeningDetails.screeningComponent[
              compIndex
            ].component.findIndex(
              (fi) => fi.screeningComponentInfo.subCompId === subcomp.subCompId
            );
            this.screeningDetails.screeningComponent[compIndex].component[
              subCombIndex
            ].compRef.address = this.screeningDetails.candidate.address;
            this.screeningDetails.screeningComponent[compIndex].component[
              subCombIndex
            ].active = true;
          }
    ...
    this.screeningDetails.screeningComponent =
      this.screeningDetails.screeningComponent
        .filter(
          (p) =>
          (p.component =
            p.compId > 0 && p.component !== undefined
              ? p.component.filter((s) => {
                if (
                  s.active === true ||
                  s.screeningComponentInfo.notApplicableFlag
                ) {
                  if (
                    s.submittedFlag &&
                    !s.screeningComponentInfo.deqcFlag
                  ) {
                    if (
                      this.screeningService.caseFlagType ===
                      this.common.VEREJECT ||
                      this.screeningService.caseFlagType ===
                      this.common.FRREJECT ||
                      this.screeningService.caseFlagType ===
                      this.common.QCREJECT ||
                      this.screeningService.caseFlagType ===
                      this.common.REOPEN
                    ) {
                      return true;
                    } else {
                      return this.userData.applicationId !== 3
                        ? false
                        : true;
                    }
                  }
                  if (s.screeningComponentInfo.notApplicableFlag) {
                    ...
                    if (this.screeningService.caseFlag) {
                      if (this.applicationId === 3) {
                        ...
                        return s.submittedFlag === false &&
                          s.preQCApproveFlag === false
                          ? true
                          : false;
                      }
                    } else if (this.screeningService.caseFlagType ===
                      this.common.NEWCASE || this.screeningService.caseFlagType ===
  ...
  saveScreeningComp(event: any) {
    if (event) {
      ...
      this.screeningDetails.document = this.screeningDocument;
      const empFresherFlag =
        event.compName.toLowerCase() ===
        (this.common.EMPLOYMENT_HR.toLowerCase() || this.common.CURRENT_EMPLOYMENT_HR.toLowerCase())&&
  ...
  commonSave(isMultiCompSubmit: any) {
    ...
    if (inSuffCount === documentUploadCount) {
      ...
      for (let i = 0; i < this.screeningDetails.document.length; i++) {
        if (this.screeningDetails.document[i].fileName) {
          formData.append(
            ...
            this.screeningDetails.document[i].document
          );
        }
      }
      for (
      ...
      if (htmlstring.length > 0) {
        if (htmlstring[0].innerHTML.includes("@@consentsignatore")) {
          htmlstring[0].innerHTML = htmlstring[0].innerHTML.replace(
            /@@consentsignatore/gi,
            this.screeningDetails.candidate.consentSignature
          );
        }
        ...
        this.screeningDetails.candidate.loadocumentHtml =
          htmlstring[0].innerHTML;
      ...
      formData.append(
        "ScreeningDetails",
        JSON.stringify(this.screeningDetails)
      );
      if (
        this.screeningService.caseFlag === true ||
        this.screeningService.caseFlagType === this.common.INSUFFCLEARANCE ||
        ...
        this.screeningService
          .addScreeningDetails(formData)
          .subscribe((resp) => {
            if (resp) {
              //Added By Megala - 20/04/2024
              if(this.screeningService.screeningCompId >0){
                this.automation.CalculateInprogressDays(this.screeningService.screeningCompId ).subscribe(resp =>{
                  console.log(resp);
                });
               }
        ...
        this.screeningService
          .addNewScreeningDetails(formData)
          .subscribe((resp) => {
            if (resp) {
              this.commonResp("case", isMultiCompSubmit, this.screeningDetails);
            }
        ...
        this.screeningService
          .addPreQCScreeningDetails(formData, "")
          .subscribe((resp) => {
            if (resp) {
              this.commonResp(
                "preQc",
      ...
      if (this.applicationId === 3) {
        ...
        if (
          ...
          if (isMultiCompSubmit) {
            ...
            const alertFlag = backList.some((x) => x.active !== true && (x.active !== false && x.submittedFlag !== false));
            if (alertFlag !== true) {
              this.backToList();
            } else if (
              alertFlag === true &&
            ...
            if (!isMultiCompSubmit && !this.screenRespFlag) {
              this.getScreeningComponentStatusDetails(
                details.screeningComponent[0].compId
              );
            }
            this.getAssignedCompDetails(this.caseNo);
            //  this.getAssignedCaseDetails(this.caseNo);
          }
        }
      }
  ...
  getScreeningComponentStatusDetails(compId: any) {
    // if ( this.screeningService.basicInfo ===true &&(this.componentName.toUpperCase() === this.common.EMPHR_EMPSUP)) {

    //      this.GetCompanyInfo();
    //     }
    if (
      (this.componentName !== this.common.EMPLOYMENT_HR &&
        this.componentName !== this.common.ADDRESS &&
        this.componentName !== this.common.PREVIOUS_EMPLOYMENT_HR &&
        this.componentName !== this.common.CURRENT_EMPLOYMENT_HR &&
        this.screeningService.caseFlag == true &&
        this.componentName !== this.common.DRUG_TEST &&
        this.componentName != this.common.CRIMINAL_DATABASE &&
        this.componentName != this.common.DATABASE_CONDUCT &&
        this.componentName != this.common.DATABASE_ADVERSE_MEDIA &&
        this.componentName !== this.common.EDUCATION) ||
      this.screeningService.caseFlag != true
    ) {
        ...
        this.screeningService
          ...
          .subscribe((res) => {
            if (res) {
              ...
              //    this.screeningService.instituteList = Object.assign([], res.institute);
              // }
              ...
              if (
                this.componentName.toUpperCase() ===
                this.common.CRIMINAL_CHECK_PCC3 ||
                this.componentName.toUpperCase() ===
                this.common.CRIMINAL_CHECK_PCC3E ||
                this.componentName.toUpperCase() ===
                this.common.CRIMINAL_CHECK_PCC1 ||
                this.componentName.toUpperCase() ===
                this.common.CRIMINAL_CHECK_PCC2 ||
...
// 30/01/2024 - changed by Vignesh M -Exist Candidate Details - showing issue solved
  getPaginationValues() {
    return {
      pageSize: 3150,
      page: 1,
      ...
      //needTotal: true,
      empInsId: 0,
      techmFlag: this.screeningService.ClientCategoryId == 4 ? true : false, //(this.screeningService.ClientCategoryId == this.screeningService.techmCatId) ? true : false,
        ...
        if (this.fileSubmission.get("candidate").valid) {
          if (this.fileSubmission.get("screening").valid) {
            if (
              ...
              const selectedComp =
                this.caseSubmissionList.screeningCaseComponent.findIndex(
    ...
    if (this.step1 !== 0) {
      if (this.step1 === 1 || this.step1 === 2) {
        if (
          ...
          this.fileSubmission.get("screening").valid
        ) {
          this.stepperChange(selectedIndex);
        } else {
          this.scroll.scrollToError();
  ...
  clientChangeEmit() {
    ...
    for (
      let first = 0;
      this.screeningService.componentList.length > first;
      first++
    ) {
      frmArray.push(
        this.initcomponentFormGroup(this.screeningService.componentList[first])
      );
    }
    for (
      let first = 0;
      ...
      if (this.screeningService.componentList[first].subCompFlag) {
        for (
          let sec = 0;
          this.screeningService.componentList[first].screeningSubComponent
            .length > sec;
          sec++
        ) {
          for (
            ...
            const subCheckFlag =
              this.screeningService.componentList[first].subCheckFlag;
        ...
        for (
          let fou = 0;
          this.screeningService.componentList[first].noOfComponent > fou;
          fou++
        ) {
          const comptype =
            this.screeningService.componentList[first].packageCount >= fou + 1
              ? "Package"
              : "Individual";
          const compInitiationDate =
            this.screeningService.componentList[first].compInitiationDate;
          compfrmarr.push(
            this.initCommonFormGroup(
              false,
              comptype,
              compId,
              null,
              this.screeningService.componentList[first].criminalCheckCount,
              deqcFlag,
              false,
              this.screeningService.componentList[first].currencyId,
              fou,
              this.screeningService.componentList[first].subCheckFlag,
              compInitiationDate
            )
      ...
      switch (this.screeningService.componentList[i].compName.toUpperCase()) {
        case this.common.PAN_CARD:
          if (this.screeningService.componentList[i].subCompFlag) {
            ...
            for (
              let k = 0;
              k < this.screeningService.componentList[i].noOfComponent;
              k++
            ) {
              ...
              compRefGroup.addControl(
                "componentCustomFields",
                this.initclientCustomFieldsForm(
                  this.screeningService.componentList[i].componentCustomFields
                )
              );
              if (this.screeningService.componentList[i].question.length > 0) {
                compRefGroup.addControl(
                  "miscQuestion",
                  this.common.initMiscForm(
                    this.screeningService.componentList[i].question
                  )
                );
              }
            }
        ...
        case this.common.ADDRESS_GEO:
          if (this.screeningService.componentList[i].subCompFlag) {
            ...
            for (
              ...
              for (
                let b = 0;
                this.screeningService.componentList[i].screeningSubComponent[a]
                  .noOfComponent > b;
                b++
              ) {
                ...
                if (this.screeningService.ClientCategoryId == 4) {
                  if (
                    this.screeningService.componentList[i].question.length > 0
                  ) {
                    addGroup.addControl(
                      "miscQuestion",
                      this.common.initMiscForm(
                ...
                if (
                  ...
                  if (
                    ...
                    this.screeningService.componentList[i]
            ...
            for (
              let c = 0;
              this.screeningService.componentList[i].noOfComponent > c;
              c++
            ) {
              if (this.screeningService.ClientCategoryId == 4) {
                if (this.screeningService.componentList[i].question.length > 0) {
          ...
          if (this.screeningService.componentList[i].subCompFlag) {
              ...
              for (
                ...
                emp.addControl(
                  ...
                  this.initEmployerForm(
                    b,
                    this.screeningService.componentList[i]
                      .screeningSubComponent[a].compId
                  )
                );
                if (
                  this.screeningService.componentList[i].question.length > 0
            ...
            for (
              ...
              compRefGroup.addControl(
                ...
                this.initEmployerForm(
                  k,
                  this.screeningService.componentList[i].compId
                )
              );
              ...
              if (this.screeningService.dAinvitationFlag === true) {
                comprefGroup.addControl(
                  "supervisorDet",
            ...
            for (
              ...
              compRefGroup.addControl(
                ...
                this.initCriminalDatabaseForm(
                  this.screeningService.componentList[i].compName.toUpperCase()
                )
              );
              compRefGroup.addControl(
                "componentCustomFields",
          ...
          if (this.screeningService.componentList[i].subCompFlag) {
            ...
            for (
              ...
              for (
                ...
                compRefGroup.addControl(
                  "compRef",
                  this.initCriminalDatabaseForm(this.screeningService.componentList[i].compName.toUpperCase())
                );
                count++;
              }
            }
        ...
        case this.common.CRIMINAL_CHECK_PCC2:
          if (this.screeningService.componentList[i].subCompFlag) {
            ...
            for (
              ...
              for (
                let a = 0;
                a < this.screeningService.componentList[i].criminalCheckCount;
                a++
              ) {
            ...
            for (
              ...
              compRefGroup.addControl(
                ...
                this.common.initGapForm(
                  this.screeningService.componentList[i].gapDetails, this.screeningService.ClientCategoryId
                )
              );
              compRefGroup.addControl(
                "componentCustomFields",
        ...
        case this.common.CV_VALIDATION:
          this.screeningService.cvValidateList =
            this.screeningService.componentList[i].cvValidationFields;
          if (this.screeningService.componentList[i].subCompFlag) {
          } else {
            for (
              let k = 0;
        ...
        case this.common.MVR:
          if (this.screeningService.componentList[i].subCompFlag) {
            ...
            for (
              ...
              const compRefGroup = compArray.controls[k] as UntypedFormGroup;
              let initform;
              if (this.screeningService.componentList[i].compName.toUpperCase() === this.common.SSN_TRACE) {
                initform = this.initSsn();
              } else if (this.screeningService.componentList[i].compName.toUpperCase() === this.common.CRIMINAL_FELONY_MISDEMEANOR_5_YEARS
                || this.screeningService.componentList[i].compName.toUpperCase() === this.common.CRIMINAL_FEDERAL_NATIONWIDE_5_YEARS) {
  ...
  rejectScreeningFileSubmission() {
    if (this.rejectComments.valid) {
      ...
      this.preQCRejectVm.screeningId =
        this.screeningDetails1.screening.screeningId;
        ...
        if (this.userData.applicationId !== 3) {
          if (
            ...
            if (data.screeningComponentInfo.notApplicableFlag === true) {
              ...
              if (
                this.userData.teamName === "DEPre-QC" ||
                (this.userData.subTeamLeadFlag === true &&
                  this.userData.teamName === "CTS-SubmissionTeam" &&
                  this.userData.teamLeadFlag !== true) ||
                this.screeningService.caseFlagType === this.common.PREQCCASE
            ...
            if (
              ...
              if (fulldata.invitationFlag === true) {
                if (
                  this.userData.subTeamName === "DEPreQC" ||
                  (this.userData.subTeamLeadFlag === true &&
                    this.userData.teamName === "CTS-SubmissionTeam" &&
              ...
              if (fulldata.invitationFlag === true && data.screeningComponentInfo.notApplicableFlag != true && data.submittedFlag != true && data.preQCApproveFlag == false && this.screeningService.caseFlagType === this.common.PREQCCASE) {
                spectFormGroup.enable();
              } else {
                if (
