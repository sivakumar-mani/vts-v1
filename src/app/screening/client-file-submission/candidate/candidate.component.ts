...
import { ClientFileSubmissionComponent } from '../client-file-submission.component';
...
export class CandidateComponent implements OnInit, OnChanges, AfterContentChecked {
  ...
  constructor(private router: Router, private cd: ChangeDetectorRef, public screeningService: ScreeningService, public common: CommonService,
    private fb: UntypedFormBuilder, private messageService: MessageService, private dialog: MatDialog, public clientFile: ClientFileSubmissionComponent
  ) {
  ...
  getCandidateList(clientId: any) {
    ...
    this.screeningService.getClientRelatedInfo(clientId).subscribe(resp => {
      if (resp) {
        ...
        const screeningGroup = this.mainForm?.get('screening') as UntypedFormGroup;
        if (screeningGroup) {
          this.siteControls =
            new AutoCompleteDropDown('Site Name', 'siteName', 'siteName', 'siteNameNO', this.clientSite,
              '', screeningGroup, false, this.clientSite.length > 0 ? false : true,
              (this.screeningService.caseFlagType === this.common.NEWCASE && this.clientSite.length > 0) ? true : false);
          this.componentControls = new AutoCompleteDropDown('Component', 'component', 'compId', 'compName', this.compList,
            '', screeningGroup, false, this.manualFilesubmissionRE, true);
        }
  ...
  isAlExist() {
    ...
    if (this.mainForm.get('screening')?.get('clientRefNo')?.valid && this.mainForm.get(this.formgroupName).get('fatherName')?.value && this.mainForm.get(this.formgroupName).get('dob')?.value && this.mainForm.get(this.formgroupName).get('firstName')?.value) {
      this.screeningService.getCandidateInfo(this.userData).subscribe(resp => {
        ...
        //this.candidateDetail = this.candidateDetail.filter()
      });
    }
  }
  backToList() {
