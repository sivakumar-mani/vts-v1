...
@Component({
  standalone: false,
  selector: 'app-common-gap-verification',
  templateUrl: './common-gap-verification.component.html',
  styleUrls: ['./common-gap-verification.component.css']
})

export class CommonGapVerificationComponent implements OnInit, OnChanges {
  compName: any;
  naFlag = false;
  nFlag = false;
  ...
  gapStatusList: any[] = [];
  gapVerList: any[] = [];
  constructor(public screeningService: ScreeningService, public common: CommonService, public fb: UntypedFormBuilder,public clientService: ClientService
  ) { }
  ...
  onSelectionChange(event: any) {
    if(this.mainForm.get('screeningComponentInfo')?.get('screeningStatusId')?.value !=true){}
    if (event > 0) {
  ...
  handleDateChangeChild(date, controlName,index) {
    ...
    // this.upperCase((this.mainForm.get("gapDetails") as UntypedFormArray).at(index).get(controlName).value, controlName);
