...
export class CommonLicenseComponent implements OnInit, OnChanges {
  ...
  public openAddNewMasterData() {
    ...
    const popupData = {
      ...
      bodyText: '',
      value: this.mainForm.get(this.formgroupName).get('issuingAuthority')?.value
    };
          ...
          if (result.Responds > 0) {
            this.master.GetLicenseAuthorityList().subscribe(resp => {
              if (resp) {
                ...
                this.filterauthorityList = resp;
                const objins = this.authorityList.find(f => f.id === result.Responds);
                this.getAuthority(objins.name);
                this.filterauthorityList.forEach(e => {
                  if (objins.name == e.name) {
                    this.filterauthorityList[0].name = objins.name;
                  }
                })
              }
