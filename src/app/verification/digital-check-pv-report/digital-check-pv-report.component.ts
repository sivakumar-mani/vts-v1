...
@Component({
  standalone: false,
  selector: 'app-digital-check-pv-report',
  templateUrl: './digital-check-pv-report.component.html',
  styleUrls: ['./digital-check-pv-report.component.css']
})
export class DigitalCheckPvReportComponent implements OnInit {
  ...
  displayedColumns = [
    ...
    { field: 'secondaryMobileNumber', header: 'Secondary Mobile Number' },
    { field: 'emailId', header: 'Email ID' },
    { field: 'latitude', header: 'Latitude' },
    { field: 'longitude', header: 'Longitude' },
    // { field: 'distance', header: 'Distance' },
    { field: 'Building / Address Status', header: 'Address Type' },
    { field: 'respondentName', header: 'RespondentName' },
...
clientNameTrigger!: MatMenuTrigger;
  ...
  constructor(private router: Router, private verificationService: VerificationService, public common: CommonService, private dateP: DatePipe) { }
  ...
  private TblAutoFilters(): void {
    this.candidateFirstNameFilteredOptions = this.candidateFirstNameFormCtrl.valueChanges.pipe(startWith(''),
      map(value =>
        (Array.from(new Set(this.digitalAddressPVReportList.map(x => x.candidateFirstName).filter(x => x))).sort())
    ...
    this.clientReferenceNoFilteredOptions = this.clientReferenceNoFormCtrl.valueChanges.pipe(startWith(''),
      map(value =>
        (Array.from(new Set(this.digitalAddressPVReportList.map(x => x.clientReferenceNo).filter(x => x))).sort())
          .filter(x => x).sort().filter(option => option.toLowerCase().includes(value))));

    this.clientNameFilteredOptions = this.clientNameFormCtrl.valueChanges.pipe(startWith(''),
      map(value =>
        (Array.from(new Set(this.digitalAddressPVReportList.map(x => x.clientName).filter(x => x))).sort())
          .filter(x => x).sort().filter(option => option.toLowerCase().includes(value))));

  }
  exportAsExcelFile() {
    this.common.exportToExcel(this.displayedColumns, this.digitalAddressPVReportList, 'DigitalAddressCheckPV_Report');
  }
  exportAsExcelFile1() {
    let tabtext = '<table border="1px">';
    let j = 0;
    const header = this.dt.columns;
    const filteredValue = this.dt.filteredValue ? this.dt.filteredValue : this.dt.value;
    ...
    const fileName = 'DigitalAddressCheckPV_Report.xls';
  ...
  showall() {
    if (this.digitalAddressPVReportList.length > 0) {
      this.itemperpage = this.digitalAddressPVReportList.length;
    }
  }
}