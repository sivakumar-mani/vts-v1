...
@Component({
  standalone: false,
  selector: 'app-scopebypass-case',
  templateUrl: './scopebypass-case.component.html',
  styleUrls: ['./scopebypass-case.component.css']
})
export class ScopebypassCaseComponent implements OnInit {
  isTechMFlag: any = '';
  ...
  btnType: boolean;
  routePath = 'Screening / Clients Case Creation / ScopeByPass Case Creation';
  isEdit: boolean;
  screenAuth: any = {};
  userData: UserData;
...
amountCtl!: ElementRef<any>;
 @ViewChild('global', { static: true }) global!: ElementRef<any>;
  displayedColumns = [
    ...
    { field: 'applicantId', header: 'Candidate Id', value: true },
 ...
 @ViewChild('uploadConfirm', { static: true }) uploadConfirm!: TemplateRef<any>;
  ...
  @ViewChild('chargeCodeTrigger', { static: true }) chargeCodeTrigger: MatMenuTrigger;
...
clientNameTrigger!: MatMenuTrigger;
  ...
  exportAsExcelFile() {
    ...
    if (this.ctsPattern.test(this.userData.deptName.toUpperCase())) {
      this.userData.filters = 'ctsFlag == true';
    } else {
      this.userData.filters = 'scopeByPassFlag== true,ctsFlag == false';
    }
  ...
  applyPagination() {
    ...
    if (this.ctsPattern.test(this.userData.deptName.toUpperCase())) {
      this.userData.filters = ((filter && filter.length > 0) ? (filter + ',') : '') + 'ctsFlag == true'
    } else {
      this.userData.filters = ((filter && filter.length > 0) ? (filter + ',') : '') + 'scopeByPassFlag== true,ctsFlag == false';
    }
  ...
  initFormGroup() {
    ...
    this.caseCreationFormGroup = this.fb.group({
      ...
      caseInitiationDateTime: [this.userData.applicationId === 1 ? new Date() : null,
      this.userData.applicationId === 1 ? Validators.required : null],
      autoAssign: [true],
      packageId: [null],
      caseComponent: [[]],
  ...
  getClients() {
    ...
    this.screeningService.getClientName(data).subscribe(resp => {
      if (resp) {
        this.clients = resp.filter(f => f.scopeByPassFlag === true);
        this.clientControls =
          new AutoCompleteDropDown('Client Name', 'clientId', 'clientId', 'clientName', this.clients,
            '', this.caseCreationFormGroup, false, false, true);
      }
  ...
  getClientId(clientId: any) {
    if (clientId) {
      this.isTechMFlag = this.clients.find(e => e.clientId === clientId).clientName
  ...
  getCaseDetailsByClientId(clientId: any) {
    this.screeningService.getDetailsByClientId(clientId).subscribe(resp => {
      if (resp) {
        ...
        this.techMcategoryId = resp.clientCategoryId;
        // if (this.invitationFlag === false) {
        //   this.caseCreationFormGroup.get('invitation')?.setValue(false);
        // }
        ...
        if (this.iqcByPassFlag == true) {
          this.caseCreationColumns = [
            ...
            { field: 'candidateMiddleName', header: 'Middle Name' },
            { field: 'candidateLastName', header: 'Last Name' },
            { field: 'casePriorityLookupId', header: 'Case Priority' },
          ...
          if (this.breadcrumbFlags.toolTip === 'Update') {
            ...
            this.packageName(this.common.tempResetData.caseEntry[0].packageId);
            // });
          }
    ...
    this.screeningService.getCaseCreationDetails(data)
      .subscribe((resp) => {
        if (resp) {
          ...
          //  resp = resp.body.filter(f => f.ctsFlag === true);
    ...
    if (this.caseCreationFormGroup.valid) {
      ...
      if (this.iqcByPassFlag === true) {
        if (this.caseCreationList.filter(e => (e.casePriorityLookupId) === 0).length > 0) {
          this.showTopCenter('warn', 'Failure Message', 'Please add Case Priority'); return;
        }
        if (this.caseCreationList.filter(e => ('' + e.clientReferenceNo.trim()).length === 0).length > 0) {
          this.showTopCenter('warn', 'Failure Message', 'Please add Transaction ID'); return;
        }
      }
      // if (( this.caseCreationList.filter(e => e.remarks === '').length > 0) && !(this.caseCreationFormGroup.controls.caseNo.value > 0)) {
      //   this.showTopCenter('warn', 'Failure Message', 'Please Enter Remarks for each candidate...'); return;
      // }
      if ((this.caseCreationFormGroup.controls.caseComponent.value.length === 0) && !(this.caseCreationFormGroup.controls
        .packageId.value) && !(this.caseCreationFormGroup.controls.caseNo.value > 0)) {
        this.showTopCenter('warn', 'Failure Message', 'Please select individual components or package..'); return;
      }
      if (this.caseDetails.scopeByPassFlag === true || this.caseDetails.ctsFlag === true) {
        // if (((this.caseCreationList.filter(e => (e.indComponentList.length === 0 && e.packageId < 1)).length > 0)
        //   || this.caseCreationList.filter(e => e.remarks === '').length > 0) && !(this.caseCreationFormGroup.controls.caseNo.value > 0)) {
        //   this.showTopCenter('warn', 'Failure Message', 'Please Select Components or Package and Enter Remarks..'); return;
        // }
        let mainFlag = true;
        // this.caseCreationList.forEach(element => {
        this.caseCreation.caseComponent = [];
        this.caseCreationFormGroup.controls.caseComponent.value.forEach(ele1 => {
          // validation
          const comp = this.packCompList && this.packCompList.length > 0 && this.packCompList.some(x => x.componentId
            === ele1.componentId && !ele1.subCompFlag === true);
          const preComp = this.preComponentList.length > 0 && this.preComponentList.some(x => x.compId
            === ele1.componentId && !ele1.subCompFlag === true) && this.caseCreationFormGroup.controls.caseNo.value > 0;
          if ((ele1.maxNoOfComp && ele1.maxNoOfComp < 2) && (comp || preComp)) {
            mainFlag = false;
          }
          if (mainFlag === true) {
            const caseComponent = new CaseComponentVm();
            caseComponent.caseComponentId = 0;
            caseComponent.compId = ele1.componentId;
            caseComponent.noOfComponent = 1;
            caseComponent.subCompFlag = ele1.subCompFlag ? ele1.subCompFlag : false;
            caseComponent.currencyId = ele1.currencyId;
            caseComponent.active = true;
            caseComponent.compName = ele1.compName;
            if (ele1.subCompFlag === true) {
              // validation
              this.packCompList = this.packCompList ? this.packCompList : [];
              const val = this.packCompList.find(x => x.componentId === ele1.componentId);
              const valPre = this.preComponentList.find(x => x.compId === ele1.componentId);
              const subComp = this.packCompList.length > 0 && val && val.packageSubComponent.length > 0
                && val.packageSubComponent.some(x => x.subCompId === ele1.packageSubComponent[0].subCompId);
              const preSubComp = this.preComponentList.length > 0 && valPre && valPre.caseSubComponent.length > 0
                && valPre.caseSubComponent.some(x => x.subCompId === ele1.packageSubComponent[0].subCompId) &&
                this.caseCreationFormGroup.controls.caseNo.value > 0;
              if ((ele1.packageSubComponent[0].maxNoOfComp && ele1.packageSubComponent[0].maxNoOfComp < 2) && (preSubComp || subComp)) {
                mainFlag = false;
              }
              const caseSubComponet = new CaseSubComponentVm();
              caseSubComponet.caseSubCompId = 0;
              caseSubComponet.subCompId = ele1.packageSubComponent[0].subCompId;
              caseSubComponet.noOfComponent = 1;
              caseSubComponet.currencyId = ele1.packageSubComponent[0].currencyId;
              caseSubComponet.active = true;
              caseSubComponet.subCompName = ele1.subCompName;
              caseComponent.caseSubComponent.push(caseSubComponet);
            } else {
              caseComponent.caseSubComponent = [];
            }
            const index = this.caseCreation.caseComponent.findIndex(x => x.compId === caseComponent.compId);
            if (index > -1) {
              this.caseCreation.caseComponent[index].caseSubComponent.push(caseComponent.caseSubComponent[0]);
            } else {
              this.caseCreation.caseComponent.push(caseComponent);
            }
            // this.caseCreationFormGroup.get('caseComponent')?.setValue(this.caseCreationFormGroup.controls.caseComponent.value);
          }
        });
        if (!mainFlag) {
      ...
      this.caseCreation.caseReceivedDate = this.userData.applicationId === 2 ? new Date() :
        this.caseCreationFormGroup.controls.receivedDateTime.value;
      this.caseCreation.caseInitiationDate = this.userData.applicationId === 2 ? new Date() :
        this.caseCreationFormGroup.controls.caseInitiationDateTime.value;
      this.caseCreation.urlName = this.caseCreationFormGroup.controls.docFolderPath.value;
      this.caseCreation.autoAssign = this.caseDetails.scopeByPassFlag === true ? true : this.caseCreationFormGroup.controls.autoAssign.value;
      ...
      this.caseCreationFormGroup.get('packageId')?.setValue((this.caseCreationFormGroup.value.caseNo > 0 && this.caseCreationFormGroup.value.packageId)
        ? null : this.caseCreationFormGroup.value.packageId);
      if (this.caseDetails.scopeByPassFlag === true) {
        this.caseCreation.packageId = this.caseCreationFormGroup.controls.packageId.value;
        // this.caseCreation.caseComponent = this.caseCreationFormGroup.controls.caseComponent.value;
      }
      this.screeningService.AddCaseCreationScope(this.caseCreation).subscribe(resp => {
  ...
  resetForm() {
    if (this.caseCreationFormGroup.controls.caseNo.value > 0) {
      ...
      this.caseCreationFormGroup.patchValue({
        ...
        receivedDateTime: this.common.getTimezoneOffset(this.common.tempResetData.caseReceivedDate, false),
        caseInitiationDateTime: this.common.getTimezoneOffset(this.common.tempResetData.caseInitiationDate, false),
  ...
  isExist(colField, index, value, header, value2) {
    ...
    if (!isExist) {
      ...
      if (!isExist) {
        ...
        } else if (colField === 'clientReferenceNo') {
  ...
  openDialogApp(src, ind, col) {
    const popupData = {
      ...
      bodyText: 'Are you sure, you want to Use the Same record?'
    };
  ...
  editCase(src: any) {
    ...
    this.screeningService.GetCaseCreationByCaseNo(src.caseNo).subscribe(res => {
      if (res) {
        ...
        setTimeout(() => {
          this.caseCreationFormGroup.patchValue({
            ...
            companySiteVisitFlag: this.common.tempResetData.companySiteVisitFlag,
            packageId: this.common.tempResetData.caseEntry[0].packageId,
            caseComponent: []
          });
      ...
      this.preComponentList = this.common.tempResetData.caseEntry[0].caseComponent;
  ...
  validateUploadFile(option: any) {
    if (this.caseCreationFormGroup.controls.caseComponent.value.length === 0 && !this.caseCreationFormGroup.controls.packageId.value) {
      this.showTopCenter('warn', 'Failure Message', 'Please select individual components or package..'); return;
    }
  ...
  saveCaseBulkUpload(option: any) {
    ...
    this.model.autoAssignFlag = this.caseDetails.scopeByPassFlag === true ? true : this.caseCreationFormGroup.controls.autoAssign.value;
    ...
    this.model.caseComponent = '';
    this.model.invitationFlag = false;
    if (this.caseCreationFormGroup.controls.packageId.value > 0) {
      this.model.packageName = this.packAndCompList.package.find(f => f.packageId === this.caseCreationFormGroup.controls.packageId.value).packageName;
    }
    if (this.caseCreationFormGroup.controls.caseComponent.value.length > 0) {
      const comp = this.caseCreationFormGroup.controls.caseComponent.value.map(m => m = (m.compName + (m.subCompFlag === true ?
        ('-' + m.packageSubComponent[0].subCompName) : '')));
      this.model.caseComponent = comp.join(',');
    }
    ...
    if ((this.caseDetails.scopeByPassFlag === true && this.iqcByPassFlag !== true) || this.caseDetails.ctsFlag === true) {
      ...
      this.master.ScopeByPassCaseCreationImportBulkNew(formData).subscribe(resp => {
  ...
  getCaseImportTemplete(scopeByPassFlag, ctsFlag) {
    ...
    let istechM: boolean = false;
    if (this.common.techmcatId === this.techMcategoryId) {
      istechM = true;
    } else {
      istechM = false;
    }
    if (this.caseCreationFormGroup.controls.clientId.value) {
      this.screeningService.getCaseImportTemplete(this.iqcByPassFlag, false, this.ExcelFlag, this.caseDetails.refNoFlag, true, istechM).subscribe(res => {
        if (res) {
