...
@Component({
  standalone: false,
  selector: 'app-finalqccheck-list',
  templateUrl: './finalqccheck-list.component.html',
  styleUrls: ['./finalqccheck-list.component.css'],
  providers: [DatePipe]
})
export class finalqccheckListComponent implements OnInit {
  @Input() filterLength: number;
  SearchCriFilter = false;
  qcSearchForm: UntypedFormGroup;
  nfFlag:boolean = false;
...
verificationIdTrigger!: MatMenuTrigger;
  ...
  applicantControl!: AutoCompleteDropDown;
  assignedList: any;
  unAssignedList: any;
  ...
  assignControl = new UntypedFormControl('Assigned');
  qcOrFqc = new UntypedFormControl('FQC');
  ...
  constructor(public commonService: CommonService, public verification: VerificationService, private qualityCheckService: QualityCheckService, private router: Router,
    public dialog: MatDialog, private message: MessageService, public screeningService: ScreeningService, private dateP: DatePipe) { }
  ngOnInit() {
    ...
    this.assignControl.setValue(((this.userData.teamLeadFlag === true || this.userData.subTeamName === 'SeniorExecutiveQC') ||
      this.userData.subTeamName === 'CRTCAMTeam' || this.userData.subTeamLeadFlag === true) ? 'Assigned' : 'Not Assigned');
      ...
      if (this.commonService.Fqc) {
        this.qualityCheckService.getFqcPendingDropdownDetails(this.userData).subscribe(res => {
          if (res) {
            ...
            this.applicantIdList = res.applicantId;
            if(this.applicantIdList.length>0){
              this.applicantIdList = this.applicantIdList.filter(f=>f.name != 'N/A')
            }
          }
        });
        this.screeningService.getPriority(this.userData).subscribe(res => {
          if (res) {
  ...
  getQualityCheckDetails() {
    ...
    this.qualityCheckService.getFQcDetails(this.userData).subscribe(resp => {
      if (resp) {
        this.shieveTotalCount = resp.headers.get('X-Total-Count');
        const res = resp.body;
        const nfData = this.commonService.CloneObject(res.getQcDetails);
        let nfList = nfData.filter(f=>f.manualFileSubmission == true)
        if(this.commonService.Fqc&&nfList!=null && nfList.length>0){
          this.nfFlag = true;
        }else{
          this.nfFlag = false;
        }
  ...
  getResList(res: any) {
    if (res) {
      ...
      // changed by Vignesh M -01/02/2024 - International QC department Re-assign team member Issue solved
      // 30-07-2024 Tech M final QC screening re assign owner issue fixed.
      if (res.clientCategoryId !== 2) {
      ...
      this.gridDataShowbyUserId(this.qcDetails);
      if (this.qualityCheckService.backFlag === true) {
  ...
  applyPagination() {
    ...
    if (this.qcSearchForm.value.prioritylookupId && this.qcSearchForm.get('prioritylookupId')?.valid) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'compPriorityLookupId ==' + this.qcSearchForm.value.prioritylookupId
    }
    if (this.qcSearchForm.value.applicantId && this.qcSearchForm.get('applicantId')?.valid) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'applicantId ==' + this.qcSearchForm.value.applicantId
    }
    ...
    if (this.userData.teamLeadFlag !== true && this.userData.subTeamLeadFlag !== true && this.userData.teamName === 'TechMQCTeam' || this.userData.teamName === 'InternationalQCTeam') {
    ...
    if (this.commonService.qcOrFqcFlag === null) {
      ...
      if (this.commonService.Fqc) {
        ...
        this.candidateControl = new AutoCompleteDropDown('Candidate Name', 'candidateName', 'id',
          ...
          this.priorityControl = new AutoCompleteDropDown('Screening Priority', 'prioritylookupId', 'id', 'name', this.priorityList,
          '', this.qcSearchForm, false, false, false, 'standard');
          this.applicantControl = new AutoCompleteDropDown('Screening ID', 'applicantId', 'name', 'name', this.applicantIdList,
          '', this.qcSearchForm, false, false, false, 'standard');
      }
      else {
  ...
  search() {
    if (this.qcSearchForm.value.clientName||this.qcSearchForm.value.prioritylookupId ||this.qcSearchForm.value.applicantId || this.qcSearchForm.value.clientRefNo || this.qcSearchForm.value.compName
  ...
  gridDataShowbyUserId(qualityCheckDetails: any) {
    if (((this.userData.teamLeadFlag === true || this.userData.subTeamName === 'SeniorExecutiveQC') ||
      ...
      if (((this.userData.teamLeadFlag === true || this.userData.subTeamLeadFlag === true || this.userData.subTeamName === 'SeniorExecutiveQC') ||
        this.userData.subTeamName === 'CRTCAMTeam') && !(this.commonService.qcOrFqcFlag === null)) {
  ...
  assignScrnOwner(data, index) {
    ...
    if (ind > -1) {
      ...
      [this.owner.persons[0], this.owner[ind]] =
  ...
  getExcelResponse(outputExcelData: any) {
    outputExcelData.map(m =>
      m.candidateFullName = m.firstName + (m.middleName ? (' ' + m.middleName) : '') +
      (m.lastName ? (' ' + m.lastName) : ''));
    ((this.userData.teamLeadFlag === true || this.userData.subTeamName === 'SeniorExecutiveQC') || this.userData.subTeamName === 'CRTCAMTeam') ? this.qcOrFqc.setValue(this.qcOrFqc.value) : this.qualityCheckList[0].finalQcFlag === true
    ...
    const fileName = this.qcOrFqc.value + ' ' + ((((this.userData.teamLeadFlag === true || this.userData.subTeamName === 'SeniorExecutiveQC') || this.userData.subTeamName === 'CRTCAMTeam') || this.userData.subTeamLeadFlag === true)
      ...
      this.qcColumn = this.qcColumn.filter(f=>f.field!='applicantId')
      if(this.nfFlag == true  && this.commonService.Fqc){
        this.qcColumn.push({ field: 'applicantId', header: 'Screening Id' })
      }
