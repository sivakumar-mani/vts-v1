...
@Component({
  standalone: false,
  selector: 'app-file-level-tracker-mis',
  templateUrl: './file-level-tracker-mis.component.html',
  styleUrls: ['./file-level-tracker-mis.component.css']
})
export class FileLevelTrackerMisComponent implements OnInit {
  screenAuth: any = {};
  userData: any;
  routePath = 'Reports / Report Tracker / File Level Tracker - MIS';
  maxDate = new Date();
 ...
 // candidateControl!: AutoCompleteDropDown;
  ...
  requestFileLevelTrackerVm = new FileLevelTrackerVm();
 ...
 @ViewChild('global', { static: true }) global!: ElementRef<any>;
  ...
  excelDataList: any[] = [];
  status=[
    //  Cmd And Added By Megala For - (sprint -16 )VTS2-2024-CRT-0170
    // {value: 'Open', viewValue: 'Open'},
    // {value: 'Pending', viewValue: 'Pending'},
    {value: 'Open and Pending', viewValue: 'Open and Pending'},
 ...
 @ViewChild('dtSeventhDayTracker', { static: true }) dtSeventhDayTracker: Table;
  ...
  ngOnInit() {
    ...
    this.initFormGroup();
    this.getFileLevelTrackerListMIS();
  }
  ...
  initautoCompleteCtrl() {
    this.clientControl = new AutoCompleteDropDown('Client Name', 'clientId', 'clientId', 'clientName', this.clientList,
      '', this.fileLevelTrackerFilterForm, false, false, false, 'standard');
          //  Cmd  By Megala For - (sprint -16 )VTS2-2024-CRT-0170
  ...
  applyPagination() {
    let filter = this.userData.filters;
     ...
     if (this.fileLevelTrackerFilterForm.value.clientRefNo && this.fileLevelTrackerFilterForm.get('clientRefNo')?.valid) {
      filter = ((filter && filter.length > 0) ? (filter + ',') : '') + 'clientRefNo@=' + this.fileLevelTrackerFilterForm.value.clientRefNo;
    }  
    this.requestFileLevelTrackerVm.pageSize = this.pageSize;
    this.requestFileLevelTrackerVm.page = this.pageNo;
    this.requestFileLevelTrackerVm.filters = filter;
    ...
    this.requestFileLevelTrackerVm.needTotal = this.needTotal;
      ...
      // Added By Megala For - (sprint -16 )VTS2-2024-CRT-0170 -180 days date validation
  chDateVal(){
    if(this.fileLevelTrackerFilterForm.controls.caseStatus.value == 'Completed'){
      this.fileLevelTrackerFilterForm.get('fromDate')?.setValidators(Validators.required);
      this.fileLevelTrackerFilterForm.get('fromDate')?.setErrors({required:true});
      this.fileLevelTrackerFilterForm.get('toDate')?.setValidators(Validators.required);
      this.fileLevelTrackerFilterForm.get('toDate')?.setErrors({required:true});
      this.fileLevelTrackerFilterForm.updateValueAndValidity();
    }else{
      this.fileLevelTrackerFilterForm.get('fromDate')?.clearValidators();
      this.fileLevelTrackerFilterForm.get('fromDate')?.setErrors(null);
      this.fileLevelTrackerFilterForm.get('toDate')?.clearValidators();
      this.fileLevelTrackerFilterForm.get('toDate')?.setErrors(null);
      this.fileLevelTrackerFilterForm.updateValueAndValidity();
      this.fileLevelTrackerFilterForm.get('toDate')?.setValue(null);
      this.fileLevelTrackerFilterForm.get('fromDate')?.setValue(null);
    }
  }
  checkPeriod(type: any) {
    if (this.fileLevelTrackerFilterForm.controls.caseStatus.value == 'Completed') {
      const fromDate = moment(this.fileLevelTrackerFilterForm.get('fromDate')?.value);
      const todate = moment(this.fileLevelTrackerFilterForm.get('toDate')?.value);
      var dfDays = todate.diff(fromDate, 'days') + 1;
      if (dfDays > 180 && this.fileLevelTrackerFilterForm.get('toDate')?.value !=null && this.fileLevelTrackerFilterForm.get('fromDate')?.value !=null ) {
        if(type == 'fromDate'){
          this.fileLevelTrackerFilterForm.get('fromDate')?.setErrors({incorrect:true});
        }else if(type == 'toDate'){
        this.fileLevelTrackerFilterForm.get('toDate')?.setErrors({incorrect:true});
        }
        this.fileLevelTrackerFilterForm.updateValueAndValidity();
      }else if(this.fileLevelTrackerFilterForm.get('toDate')?.value !=null &&  this.fileLevelTrackerFilterForm.get('fromDate')?.value !=null ){
        this.fileLevelTrackerFilterForm.get('toDate')?.setErrors(null);
        this.fileLevelTrackerFilterForm.get('fromDate')?.setErrors(null);
        this.fileLevelTrackerFilterForm.updateValueAndValidity();
      }
  ...
  // Ended By Megala For - (sprint -16 )VTS2-2024-CRT-0170 -180 days date validation
  //Added By Megala
  getFileLevelTrackerListMIS() {
    this.screeningId = 0;
    ...
    if(this.fileLevelTrackerFilterForm.get('clientId')?.value > 0){
    const flClientLst = this.clientList.filter(f=>f.clientId == this.fileLevelTrackerFilterForm.get('clientId')?.value );
    this.iQCApprovedDateInFLTFlaglst = this.fileLevelTrackerFilterForm.get('clientId')?.value ?
    flClientLst : [];
    }
    if(this.iQCApprovedDateInFLTFlaglst != null && this.iQCApprovedDateInFLTFlaglst.lenght>0){
    ...
    this.requestFileLevelTrackerVm.compId = [];
    const clId = this.fileLevelTrackerFilterForm.get('clientId')?.value ?
      this.fileLevelTrackerFilterForm.controls.clientId.value :0;
      if(clId > 0 ){
      this.requestFileLevelTrackerVm.clientId.push(clId);
      }
    ...
    this.reportService.GetFileLevelTrackerList(this.requestFileLevelTrackerVm).subscribe(resp => {
       this.requestFileLevelTrackerVm.clientId = []
       this.requestFileLevelTrackerVm.clientRefNo=""
      if (resp) {
        ...
        this.selectedRow = null;
        this.filelevelClientno = [];
        this.fileLevelTrackerlist = [];
  ...
  getResp(res: any) {
    ...
    this.getFileLevelTrackerDetailsMis(this.screeningId);
    //this.searchControl();
  }
  getFileLevelTrackerDetailsMis(screeningId: number) {
    this.screeningId = 0;
    if (screeningId != 0) {
      const currentDateTime = new Date();
      const fromDate = this.fileLevelTrackerFilterForm.get('fromDate')?.value ? new Date(this.fileLevelTrackerFilterForm.get('fromDate')?.value) : null;
      ...
      this.requestFileLevelTrackerVm.compId = [];
         // Cmd By Megala For - (sprint -16 )VTS2-2024-CRT-0170 
      //  this.fileLevelTrackerFilterForm.get('component')?.value ?
...
// Added by megala -25-03-2024
  getFileLevelTrackerListExport() {
    ...
    //   this.fileLevelTrackerFilterForm.controls.component.value.map(m => m.id) : [];
    const clId = this.fileLevelTrackerFilterForm.get('clientId')?.value ?
      this.fileLevelTrackerFilterForm.controls.clientId.value :[];
      if(clId > 0 ){
      this.requestFileLevelTrackerVm.clientId.push(clId);
      }
      this.requestFileLevelTrackerVm.caseStatus= this.fileLevelTrackerFilterForm.get('caseStatus')?.value ? 
      this.fileLevelTrackerFilterForm.get('caseStatus')?.value:"";
      this.requestFileLevelTrackerVm.clientRefNo = this.fileLevelTrackerFilterForm.get('clientRefNo')?.value ? 
      this.fileLevelTrackerFilterForm.get('clientRefNo')?.value:"";    
    this.requestFileLevelTrackerVm.loginUserDetVm = this.userData;
    this.requestFileLevelTrackerVm.applyPaging = false;
    this.reportService.GetSeventhDayTracker(this.requestFileLevelTrackerVm).subscribe(resp => {
      ...
      if( this.selectedRow !== null && this.selectedRow.clientCategoryId===4){
  ...
  selectDetail(screeningId: any) {
    this.screeningId = 0;
    this.getFileLevelTrackerDetailsMis(screeningId);
  }
  ...
  getDynamicColumns(data: any) {
    ...
    if(this.iQCApprovedDateInFLTFlaglst != undefined && this.iQCApprovedDateInFLTFlaglst.length>0){
